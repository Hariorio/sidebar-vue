                    ...
                    <v-container>
                        <v-row>
                            <v-col cols="12">
                                ...
                                <p class="text-center">
                                    Are you sure delete this Deposit ？
                                </p>
                            </v-col>
                        </v-row>
                    </v-container>
                ...
                <v-card-text>
                    <v-container>
                        <v-row class="mt-2 mb-5">
                            ...
                            <v-col class="col-12" sm="6" md="4">
                                <h6 class="blue-lcd mb-1">Account Num</h6>
                                <v-autocomplete
                                    dense
                                    solo
                                    clearable
                                    ...
                                    class="ma-0 pa-0 border-12"
                                    hide-details=true
                                    @change="(event) => getDetailAcNum(event)"
                                ></v-autocomplete>
                            </v-col>
                            <v-col class="col-12" sm="6" md="6">
...
export default {
    ...
    methods: {
        ...
        detailItemEdit(item){
            ...
            this.getAcNum()
            this.detailOtherDeposit()
        },
        detailItemDelete(item){
            this.detail = item
            this.getBranchOffice()
            this.entity_id = item.entity_id
            this.office_id = item.office_id
        ...
        async getPullData(){
            ...
            var respData = await other_payment_deposit.index(`?search=${this.search ? this.search : ''}&tr_type=OD&entity_id=${this.entity_id ? this.entity_id : ''}&office_id=${this.office_id ? this.office_id : ''}&ac_num=${this.ac_num ? this.ac_num : ''}&tr_date_start=${start_date}&tr_date_end=${end_date}&bidang=${this.$store.state.user.bidang ? this.$store.state.user.bidang : ''}&komisi=${this.$store.state.user.komisi ? this.$store.state.user.komisi : ''}&flag_three=${this.$store.state.user.flag_three ? this.$store.state.user.flag_three : ''}&gl_flag_id=${this.$store.state.user.id_pdt ? this.$store.state.user.id_pdt : ''}`, null, false, false, false)
            if (respData.status === 200) {
                this.other_deposits = respData.data.data
                this.$store.dispatch('setOverlay', false)
        ...
        async getAccountBank(){
            var respData = await account_bank.index('', null, false, false, false)
            if (respData.status === 200) {
        ...
        async getDetailAcNum(event, index){
            if (event === null || event === '') {
                ...
                var ac_num = this.detail_items.filter(i => i.ac_num === event)
                ...
                if (ac_num.length > 1) {
                    ...
                    if (respData.status === 200) {
                        console.log(respData.data.data[0]);
        ...
        async submit(){
            ...
            if (this.total == 0 || this.entity_id === '' || this.office_id === '' || this.bank_account === '') {
                this.snackbar = {
                    color: "error",
                    icon: "mdi-alert-circle",
                    mode: "multi-line",
                ...
                this.credits.push(
                    {
                        ...
                        ac_num_cr: this.detail_items[index].ac_num,
                        amt_cr: this.detail_items[index].amount,
                        is_del: 0
                    }
                )
            }
        ...
        async exportExcel(){
            ...
            var respData = await other_payment_deposit.index(`?search=${this.search ? this.search : ''}&tr_type=OD&entity_id=${this.entity_id ? this.entity_id : ''}&office_id=${this.office_id ? this.office_id : ''}&ac_num=${this.bank_acc ? this.bank_acc : ''}&tr_date_start=${start_date}&tr_date_end=${end_date}&bidang=${this.$store.state.user.bidang ? this.$store.state.user.bidang : ''}&komisi=${this.$store.state.user.komisi ? this.$store.state.user.komisi : ''}&flag_three=${this.$store.state.user.flag_three ? this.$store.state.user.flag_three : ''}&gl_flag_id=${this.$store.state.user.id_pdt ? this.$store.state.user.id_pdt : ''}`, null, false, false, false)
            if (respData.status === 200) {
