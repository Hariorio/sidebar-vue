                        ...
                        <v-row class="mt-2 mb-5">
                            ...
                            <v-col class="col-12" sm="6" md="6">
                                ...
                                <v-autocomplete
                                    ...
                                    class="ma-0 pa-0 border-12"
                                    hide-details=true
                                    @change="(event) => showLedgerParent(event)"
                                ></v-autocomplete> 
                            </v-col>
                            <v-col class="col-12" sm="6" md="6">
                                <h6 class="blue-lcd mb-1">AC Num Sub Parent <span class="red--text"><strong>* </strong></span></h6>
                                <v-text-field
                                    solo
                                    dense
                                    label="AC Num"
...
export default {
    data() {
        return {
            ...
            headers:[
                ...
                { text: 'AC Num Sub Parent', value: 'ac_sub_par_one' , align:'left'},
                { text: 'AC Desc', value: 'ac_sub_par_one_desc' , align:'left'},
            ...
            headersColumn: {
                ...
                'Office': 'office',
                'AC Par': 'ac_par',
                'AC Num Sub Parent': 'ac_sub_par_one',
                'AC Desc': 'ac_sub_par_one_desc',
                'Dr Cr': 'drcr'
            },
            ac_par: '',
            ac_parents: [],
    ...
    methods: {
        ...
        detailItem(item){
            ...
            this.ac_num = item.ac_sub_par_one
            this.ac_name = item.ac_sub_par_one_desc
            this.drcr = item.drcr
        },
        detailItemEdit(item){
            this.detail = item
        ...
        async showLedgerParent(event){
            if (event === null || event === '') {
                this.ac_par = ''
            } else{
                var respData = await ledger_parent.fetchLedgerParent(`?entity_id=${this.entity_id ? this.entity_id : ''}&office_id=${this.office_id ? this.office_id : ''}&ac_num=${this.ac_par ? this.ac_par : ''}`, null, false, false, false)
                if (respData.status === 200) {
        ...
        async getPullData(){
            this.ledger_sub_parents = []
            this.$store.dispatch('setOverlay', true)
            var subpath = ''
            if (this.search) {
                subpath = this.search ? `?search=${this.search}` : ''
            }
            var respData = await ledger_sub_parent.fetchLedgerSubParent(`?search=${this.search ? this.search : ''}&entity_id=${this.entity_id ? this.entity_id : ''}&office_id=${this.office_id ? this.office_id : ''}&ac_par=${this.ac_par ? this.ac_par : ''}`, null, false, false, false)
            if (respData.status === 200) {
                this.ledger_sub_parents = respData.data.data
        ...
        async submit(){
            ...
            if (this.entity_id === '' || this.office_id === '' || this.ac_par === '' || this.ac_num === '' || this.ac_name == '' || this.drcr == '' ) {
                this.snackbar = {
                    color: "error",
            ...
            var reqBody = {
                ...
                'ac_par': this.ac_par,
                'ac_sub_par_one': this.ac_num,
                'ac_sub_par_one_desc': this.ac_name,
                'drcr' : this.drcr
            }

            const respData = await ledger_sub_parent.storeLedgerSubParent('', reqBody, false, false, false)
        ...
        async submitEdit(){
            ...
            const respData = await ledger_sub_parent.updateLedgerSubParent('', reqBody, false, false, false)
